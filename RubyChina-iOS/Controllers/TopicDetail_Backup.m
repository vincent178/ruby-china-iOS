...
@implementation TopicDetail_Backup
...
- (void)handleTopicDetailData:(NSDictionary *)topicDetail {
    ...
    if (topicAuthor && nodeName && timeAgoString) {
        self.authorPostedTimeAgo = [NSString stringWithFormat:@"%@ posted at %@ %@", topicAuthor, nodeName, timeAgoString];
    }
    ...
    NSString *replacedHTMLString = [[topicHTMLString stringByReplacingOccurrencesOfString:@"\n" withString:@"<br>"]
                                    stringByReplacingOccurrencesOfString:@"\b" withString:@"    "];
    NSData *htmlData = [replacedHTMLString dataUsingEncoding:NSUTF8StringEncoding];
    self.attributedTopicDetail = [[NSAttributedString alloc] initWithHTMLData:htmlData documentAttributes:nil];
    
   
    // topic replies data
    self.topicRepliesData = self.topicDetailData[@"replies"];
    NSLog(@"self.topicRepliesData.count: %ld", (unsigned long)self.topicRepliesData.count);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        RCReplyCell *topicReplyCell = [tableView dequeueReusableCellWithIdentifier:replyIdentifier];
        
        if (topicReplyCell == nil) {
            topicReplyCell = [[RCReplyCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:replyIdentifier];
